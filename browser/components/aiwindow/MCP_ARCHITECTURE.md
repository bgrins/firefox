...
This document describes the architecture for integrating MCP (Model Context Protocol) support into Firefox's aiwindow component. The design supports both remote HTTP servers and sandboxed JavaScript servers running in-browser.
...
└───────┬─────────────────────┬───────────────────────────┘
        ...
        ▼                     ▼
┌──────────────────┐  ┌──────────────────────────┐
│  HTTP Transport  │  │   Sandbox Transport      │
│  (remote)        │  │   (in-browser JS)        │
└─────────┬────────┘  └───────────┬──────────────┘
          │                       │
          │                       │ Cu.Sandbox
          ▼                       ▼
┌──────────────────┐  ┌──────────────────────────┐
│  Remote MCP      │  │   Cu.Sandbox             │
│  Server          │  │   - MCP server JS code   │
│  (HTTP/SSE)      │  │   - Message passing      │
│                  │  │   - Restricted APIs      │
└──────────────────┘  └──────────────────────────┘
```
...
│   ├── OllamaProvider.sys.mjs (new)
│   └── LLMProvider.sys.mjs (new)
├── services/
│   ├── mcp/
│   │   ├── MCPServerManager.sys.mjs (new)
│   │   ├── MCPHttpTransport.sys.mjs (new)
│   │   ├── MCPSandboxTransport.sys.mjs (new)
│   │   ├── MCPToolRegistry.sys.mjs (new)
│   │   ├── MCPCapabilityProvider.sys.mjs (new)
│   │   ├── moz.build
│   │   └── servers/
│   │       ├── filesystem.js (example sandbox server)
│   │       └── memory.js (example sandbox server)
│   └── permissions/
│       ├── MCPPermissionManager.sys.mjs (new)
│       └── moz.build
└── ui/
    ├── content/
    │   ├── mcp-settings.html (new)
    │   └── mcp-settings.js (new)
    └── components/
        └── mcp-permission-prompt/ (new)
```
