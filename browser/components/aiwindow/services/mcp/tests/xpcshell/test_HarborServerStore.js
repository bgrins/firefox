...
add_task(async function test_save_extension_server() {
  HarborServerStore.clearAll();

  const config = {
    name: "Extension MCP Server",
    type: "extension",
    extensionId: "mcp-server@example.com",
    enabled: true,
  };
  ...
  Assert.ok(saved.id, "Should generate an ID");
  Assert.equal(saved.name, "Extension MCP Server", "Should save name");
  Assert.equal(saved.type, "extension", "Should save type");
  Assert.equal(saved.extensionId, "mcp-server@example.com", "Should save extensionId");
  Assert.equal(saved.code, null, "Code should be null for extension servers");
  Assert.equal(saved.url, null, "URL should be null for extension servers");
  Assert.equal(saved.enabled, true, "Should save enabled state");

  const servers = HarborServerStore.loadServers();
  Assert.equal(servers.length, 1, "Should have one server");
  Assert.equal(servers[0].extensionId, "mcp-server@example.com", "extensionId should match");
});

add_task(async function test_save_extension_requires_extensionId() {
  HarborServerStore.clearAll();

  Assert.throws(
    () => HarborServerStore.saveServer({ name: "Test", type: "extension" }),
    /Extension servers require an extensionId/,
    "Extension servers should require extensionId"
  );
});

add_task(async function test_all_server_types() {
  HarborServerStore.clearAll();

  // Create one of each type
  const sandbox = HarborServerStore.saveServer({
    name: "Sandbox Server",
    type: "sandbox",
    code: "test code",
  });

  const http = HarborServerStore.saveServer({
    name: "HTTP Server",
    type: "http",
    url: "http://localhost:3000",
  });

  const extension = HarborServerStore.saveServer({
    name: "Extension Server",
    type: "extension",
    extensionId: "test-ext@example.com",
  });

  const servers = HarborServerStore.loadServers();
  Assert.equal(servers.length, 3, "Should have 3 servers");

  const types = servers.map(s => s.type).sort();
  Assert.deepEqual(types, ["extension", "http", "sandbox"], "Should have all types");

  Assert.equal(
    HarborServerStore.getServer(sandbox.id).code,
    "test code",
    "Sandbox server has code"
  );
  Assert.equal(
    HarborServerStore.getServer(http.id).url,
    "http://localhost:3000",
    "HTTP server has URL"
  );
  Assert.equal(
    HarborServerStore.getServer(extension.id).extensionId,
    "test-ext@example.com",
    "Extension server has extensionId"
  );
});
