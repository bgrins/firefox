...
const BUILTIN_SERVERS = [
  { id: "builtin-hello", name: "Hello Server", type: "sandbox", enabled: true, builtin: true },
  { id: "builtin-time", name: "Time Server", type: "sandbox", enabled: true, builtin: true },
  { id: "builtin-browser-tabs", name: "Browser Tabs", type: "sandbox", enabled: true, builtin: true, capabilityLevel: "browser-readonly" },
  { id: "builtin-history", name: "History Search", type: "sandbox", enabled: true, builtin: true, capabilityLevel: "browser-readonly" },
  { id: "builtin-security-test", name: "Security Test", type: "sandbox", enabled: true, builtin: true },
];
...
class HarborUI {
  ...
  async init() {
    console.log("[Harbor] Initializing...");

    // Set up Ollama prefs if not already set
    ...
    try {
      ...
      console.log("[Harbor] MCP infrastructure initialized");
    } catch (error) {
    ...
    // Load servers from store and register them
  ...
  updateModelInfo() {
    ...
    try {
      ...
      // Check if we can reach the endpoint and get models
      if (endpoint) {
  ...
  onModelSelected(event) {
    ...
    if (model) {
      ...
      console.log(`[Harbor] Model changed to: ${model}`);
    }
  }
  ...
  async loadServersFromStore() {
    ...
    // First, load built-in servers (always available, not stored in pref)
    for (const builtin of BUILTIN_SERVERS) {
      // Skip if disabled
      if (disabledServers.includes(builtin.id)) {
        console.log(`[Harbor] Skipping disabled builtin server: ${builtin.id}`);
        continue;
      }
      ...
      try {
        ...
        // Set capability profile if specified
        if (builtin.capabilityLevel) {
          const profile = lazy.CapabilityProfileStore.getDefaultProfile(builtin.capabilityLevel);
          if (profile) {
            lazy.CapabilityProfileStore.save(builtin.id, profile);
          }
        }
      } catch (error) {
        console.error(`[Harbor] Failed to register builtin server ${builtin.id}:`, error);
      }
    }

    // Then load user-added servers from pref
    const userServers = lazy.HarborServerStore.loadServers();
    for (const config of userServers) {
      // Skip if disabled
      if (disabledServers.includes(config.id)) {
        console.log(`[Harbor] Skipping disabled server: ${config.id}`);
        continue;
      }
      ...
      if (config.enabled) {
        try {
          ...
          console.error(`[Harbor] Failed to register server ${config.id}:`, error);
        }
      }
    }
  }
  ...
  getAllServers() {
    // Combine builtin servers with user servers
    const builtinConfigs = BUILTIN_SERVERS.map(b => this.getBuiltinServerConfig(b));
    const userServers = lazy.HarborServerStore.loadServers();
  ...
  getDisabledServers() {
    try {
      const pref = Services.prefs.getStringPref("browser.aiwindow.harbor.disabled", "[]");
  ...
  setDisabledServers(disabledIds) {
    Services.prefs.setStringPref("browser.aiwindow.harbor.disabled", JSON.stringify(disabledIds));
  }
  ...
  async toggleServerEnabled(serverId) {
    ...
    if (isCurrentlyDisabled) {
      ...
      // Stop and unregister the server
  ...
  async registerServerFromConfig(config) {
    ...
    if (config.type === "sandbox" && config.code) {
      ...
      // Only refresh tools if server started successfully
      const serverStatus = this.manager.getServerStatus(config.id);
      if (serverStatus === "running") {
        await this.registry.refreshServerTools(config.id);
        console.log(`[Harbor] Registered server: ${config.name} (${config.id})`);
      } else {
        console.warn(`[Harbor] Server registered but not running: ${config.name} (status: ${serverStatus})`);
      }
      ...
      // Get bearer token from secure credential store if available
      ...
      if (serverStatus === "running") {
        await this.registry.refreshServerTools(config.id);
        console.log(`[Harbor] Registered HTTP server: ${config.name} (${config.id})`);
      } else {
        console.warn(`[Harbor] HTTP server registered but not running: ${config.name} (status: ${serverStatus})`);
      }
    }
  }
  ...
  async sendMessage() {
    ...
    try {
      ...
      console.log(`[Harbor] Using ${mcpToolsConfig.length} MCP tools only`);

      // Make MCP registry available to Chat for tool execution
      ...
      // Stream response from Ollama with MCP tools only (no built-in tools)
      let assistantText = "";
      console.log("[Harbor] Streaming response from Ollama...");

      for await (const chunk of lazy.Chat.fetchWithHistory(conversation, {
        tools: mcpToolsConfig,
        toolMap: {}, // Empty - all tools handled via MCP registry
      })) {
        // Handle text chunks
      ...
      console.log("[Harbor] Streaming complete");
  ...
  renderServers() {
    ...
    for (const config of allServers) {
      ...
      // Show "disabled" status if disabled, otherwise show runtime status
      let status;
      if (isDisabled) {
        status = "disabled";
      } else {
        const runtimeServer = this.manager.servers.get(config.id);
        status = runtimeServer ? runtimeServer.status : "stopped";
      }

      const statusSpan = document.createElement("span");
      statusSpan.className = `server-status ${status.toLowerCase()}`;
      ...
      // Add control buttons (Edit only, for user servers)
      if (!config.builtin) {
        const controlsDiv = document.createElement("div");
        controlsDiv.className = "server-item-controls";

        const editBtn = document.createElement("button");
        editBtn.textContent = "Edit";
        editBtn.addEventListener("click", e => {
          e.stopPropagation();
          this.openServerDialog(config.id);
        });
        controlsDiv.appendChild(editBtn);
        serverDiv.appendChild(controlsDiv);
      }
  ...
  async executeSelectedTool() {
    ...
    try {
      console.log(
        `[Harbor] Executing tool: ${this.selectedTool.fullyQualifiedName}`,
        args
      );
  ...
  openServerDialog(serverId = null) {
    const dialog = document.getElementById("server-editor-dialog");
    const form = document.getElementById("server-editor-form");
    ...
    if (serverId) {
      // Edit mode - hide template selector
      ...
      if (config) {
        ...
        document.getElementById("server-type").value = config.type;
        document.getElementById("server-enabled").checked = config.enabled;

        if (config.type === "sandbox") {
          const code = config.code || "";
          document.getElementById("server-code").value = code;
          ...
          document.getElementById("server-url").value = config.url || "";
          // Show placeholder if token exists - we don't read actual token for security
      ...
      document.getElementById("token-field").classList.add("hidden");
      document.getElementById("http-actions").classList.add("hidden");
      document.getElementById("server-token").placeholder =
        "Optional bearer token";
  ...
  async saveServer() {
    ...
    if (type === "sandbox") {
      ...
      // Only set hasBearerToken flag - actual token stored securely
    ...
    try {
      ...
      console.log(
        `[Harbor] Server saved: ${saved.name} (${saved.id}) with capability level: ${capabilityLevel}`
      );
    } catch (error) {
      console.error("[Harbor] Failed to save server:", error);

      // Show inline error for code-related errors
  ...
  async deleteServer(serverId) {
    ...
    try {
      // Unregister the server if registered
      if (this.manager.servers.has(serverId)) {
        await this.manager.unregisterServer(serverId);
      }
      ...
      console.log(`[Harbor] Server deleted: ${serverId}`);
    } catch (error) {
  ...
  exportServers() {
    ...
    try {
      ...
      console.log("[Harbor] Servers exported");
    } catch (error) {
  ...
  async resetToDefaults() {
    ...
    try {
      // Unregister user servers (not builtins)
      const userServers = lazy.HarborServerStore.loadServers();
      for (const config of userServers) {
      ...
      // Clear user servers and their capability profiles
      lazy.HarborServerStore.clearAll();

      // Clear only non-builtin capability profiles
      for (const config of userServers) {
      ...
      console.warn("[Harbor] Reset to defaults complete");
    } catch (error) {
  ...
  async importServers(event) {
    ...
    try {
      ...
      // Register newly imported servers
      ...
      console.log(`[Harbor] Imported ${count} servers`);
    } catch (error) {
