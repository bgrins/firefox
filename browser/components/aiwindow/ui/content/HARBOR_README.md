...
- **Extension Servers**: WebExtension-based MCP servers using browser.harbor API
...
Extension servers are WebExtensions that implement MCP servers using the `browser.harbor` API. Unlike sandbox servers (which run in Cu.Sandbox with restricted capabilities), extension servers run with full WebExtension privileges.

### Creating an Extension Server

1. Create a WebExtension with the `harbor` permission:
...
2. Implement the MCP protocol in your background script:

```javascript
// Add message listener first (required before registration)
...
browser.harbor.registerMCPServer({
  name: "my-server",
  version: "1.0.0",
...
- `registerMCPServer(metadata)` - Register as an MCP server
- `unregisterMCPServer()` - Unregister from Harbor
- `sendMCPResponse(requestId, response)` - Send response to a request
- `onMCPMessage` - Event fired when Harbor sends a message

### Extension vs Sandbox Servers

| Feature | Sandbox Servers | Extension Servers |
|---------|-----------------|-------------------|
| Isolation | Cu.Sandbox (null principal) | WebExtension process |
| Capabilities | Limited via CapabilityBridge | Full extension permissions |
| Code editing | Built-in editor in Harbor | Requires extension reload |
| Use case | Simple tools, templates | Complex integrations |

See `browser/components/aiwindow/extensions/time-server-mcp/` for a complete example.
...
MCPServerManager + MCPToolRegistry
    |
    +-- MCPSandboxTransport ---> Cu.Sandbox (JavaScript MCP servers)
    |
    +-- MCPHttpTransport ------> Remote HTTP servers
    |
    +-- MCPExtensionTransport -> WebExtension MCP servers
                |
          HarborMCPRegistry <--> ext-harbor.js <--> browser.harbor API
```
...
+-- schemas/harbor.json               - browser.harbor API schema
+-- parent/ext-harbor.js              - browser.harbor implementation
+-- test/browser/browser_ext_harbor.js - Extension API tests
```
