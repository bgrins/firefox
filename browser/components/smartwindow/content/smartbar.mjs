...
export function attachToElement(element, options = {}) {
  ...
  function createSuggestionButton(suggestion, index) {
    ...
    let iconElement;
    if (
      (suggestion.icon && suggestion.icon.startsWith("data:")) ||
      (suggestion.icon && suggestion.icon.startsWith("http"))
    ) {
      ...
      iconElement.textContent =
        suggestion.icon ||
        (getQueryTypeIcon ? getQueryTypeIcon(suggestion.type) : "🔍");
    }
    ...
    text.textContent =
      suggestion.label || suggestion.text || suggestion.title || "";
    ...
    button.addEventListener("click", e => {
      ...
      if (currentMentionContext) {
        ...
        editor
          ...
          .focus()
          .deleteRange({
            from: currentMentionContext.start,
            to: currentMentionContext.end,
          })
          .run();
