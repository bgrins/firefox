...
- `./mach build`: Build the project. Full builds can take a long time, up to tens of minutes. By default, this will only output warnings and errors when the CLAUDECODE environment variable is set, with the full log written to a file you can keep an eye on. You can use `--show debug` to see all output, or `--show info/warning/error` to filter by log level.
